...
const NotiItem: React.FC<NotiItemProps> = ({
  ...
  return (
    <div
      className={`p-6 flex w-full justify-between gap-3 text-white hover:bg-button ${
        unread === true ? "bg-basic border border-border-color" : ""
      }`}
    >
      ...
      <div className="text-left w-[80%]">
        <div className="flex justify-between w-full">
          ...
          <div className="flex justify-center items-center gap-1">
            <div
              className={`${
                unread === true ? "w-2 h-2 rounded-full bg-[#7819F3]" : "hidden"
              }`}
            ></div>
            <span className="text-xs text-[#b9b9b933]">{alerttime}</span>
          </div>
        </div>
        <span className="text-sm text-[#b9b9b933]">{content}</span>
      </div>
    </div>
  );
};
